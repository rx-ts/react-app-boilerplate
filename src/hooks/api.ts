...
import { useEnhancedEffect } from './lifecycle'

import { ApiMethod, NO_CONTENT } from 'shared'
import type { Nilable, URLSearchParamsOptions } from 'types'
import { CONTENT_TYPE, isObservableLike, normalizeUrl } from 'utils'
  ...
  return invokeRequestInterceptors(req).pipe(
    switchMap(req => {
      ...
      return fromFetch<Response>(normalizeUrl(url, query), {
        ...rest,
        body: json ? JSON.stringify(body) : (body as Nilable<BodyInit>),
        selector: res => of(res),
      }).pipe(
...
export function useApi<T, E = api.Error>(url?: string): UseApiResult<T, E>
